...
<form id="formID" name="formID" method="post" action="exam-score-entry.php">
...
while($row=mysqli_fetch_array($_SQL_2,MYSQLI_ASSOC)){
	...
	echo "</td>";
	echo "<td align='center'>";
	echo "<a href='exam-score-entry.php?class_ID=$row[class_entryid]&term_ID=$row[termname]&batch_ID=$row[batchid]&subject_ID=$row[subjectid]'><i class='fa fa-plus' style='color:blue'></i></a>";
	echo "</td>";
	echo "</tr>";
	}
echo "</table>";
...
<td width="70%">
<div class="form-entry">
	<form id="formID2" name="formID2" method="post" action="exam-score-entry.php">
<?php
...
echo "<button id='save_all_mark' name='save_all_mark' class='button-save'><i class='fa fa-save'></i> SAVE EXAM SCORE</button>";
echo "</td>";
echo "</tr>";
echo "</tbody>";
echo "</table>";
