...
<form id="formID" name="formID" method="post" action="scores-report-all.php">
<h4>SUBJECTS</h4>
<?php	
...
//	echo "<option value='$row[class_entryid]'>$row[class_name]:Term: $row[termname] $row[subject]</option>";
		//echo "<div style='padding:5px;background-color:#eee'><a style='color:royalblue;' href='scores-report-all.php?admin_class_id=$row[class_entryid]&term_id=$row[termname]&subject_id=$row[subjectid]&batchid=$row[batchid]'><i class='fa fa-plus' style='color:darkgreen'></i> $row[class_name]:Semester: $row[termname] $row[subject] - $row[batch]</a></div><br/>";
			echo "<div style='padding:5px;background-color:#eee'><a style='color:royalblue;' href='scores-report-all.php?class_id=$row[class_entryid]&term_id=$row[termname]&subject_id=$row[subjectid]&batchid=$row[batchid]'><i class='fa fa-plus' style='color:darkgreen'></i> $row[class_name]:Semester: $row[termname] $row[subject] - $row[batch]</a></div><br/>";
...
if(mysqli_num_rows($_SQL_EXECUTE)==0){
	...
	echo "<a onclick=\"javascript:return confirm('Do you to delete mark?')\" title='Delete score: $row[mark]' href='scores-report-all.php?delete_mark=$row[markid]'><i class='fa fa-trash-o' style='color:red'></i></a>";
	echo "</td>";
