...
$_SQL_BIL=mysqli_query($con,"SELECT SUM(ip.price) AS Total_Amount,transactionid FROM tblbilling bi INNER JOIN tblitemprice ip ON bi.itempriceid=ip.itempriceid WHERE bi.userid='$_SESSION[USERID]' AND ip.class_entryid='$row_tr[class_entryid]' AND ip.term='$row_tr[termname]'");
...
WHERE bi.userid='$_SESSION[USERID]' AND ip.class_entryid='$row_class[class_entryid]' AND ip.term='$row_tr[termname]' ORDER BY pm.datetimepayment DESC";
...
$_SQL_BIL=mysqli_query($con,"SELECT SUM(ip.price) AS Total_Amount FROM tblbilling bi INNER JOIN tblitemprice ip ON bi.itempriceid=ip.itempriceid WHERE bi.userid='$_SESSION[USERID]' AND ip.class_entryid='$row_tr[class_entryid]' AND ip.term='$row_tr[termname]'");
if($row_b=mysqli_fetch_array($_SQL_BIL,MYSQLI_ASSOC)){
echo "<tr>";
