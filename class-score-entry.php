...
<form id="formID" name="formID" method="post" action="class-score-entry.php">
...
while($row=mysqli_fetch_array($_SQL_2,MYSQLI_ASSOC)){
	...
	echo "</td>";
	echo "<td align='center'>";
	echo "<a href='class-score-entry.php?class_ID=$row[class_entryid]&term_ID=$row[termname]&batch_ID=$row[batchid]&subject_ID=$row[subjectid]'><i class='fa fa-plus' style='color:blue'></i></a>";
	echo "</td>";
	echo "</tr>";
	}
echo "</table>";
...
<td width="70%">
<div class="form-entry">
<form id="formID2" name="formID2" method="post" action="class-score-entry.php">
<?php
...
echo "</caption>";
echo "<thead><th><input type='checkbox' id='all' name='all' Onclick='selectAll()'></th><th>*</th><th>STUDENT</th><th>MARK</th></thead>";
echo "<tbody>";
...
$_SQL=mysqli_query($con,"SELECT * FROM tblmark mk WHERE mk.userid='$row[userid]' AND mk.testtype='Class Score'  AND mk.assignmentid='$row[assignmentid]'");
	...
	echo "<input type='text' style='text-align:center' id='marks' name='marks[]' value='' placeholder='Enter Mark' class='validate[custom[number]]'/>";
	echo "</td>";
	echo "</tr>";
	}
}	
...
echo "<button id='save_all_mark' name='save_all_mark' class='button-save'><i class='fa fa-save' style='color:white'></i> SAVE SCORE</button>";
echo "</td>";
echo "</tr>";
echo "</tbody>";
echo "</table>";
