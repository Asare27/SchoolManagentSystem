...
<form id="formID" name="formID" method="post" action="upload-class-score-entry.php">
	<h4>GROUP CLASS SCORE UPLOAD</h4>
<?php	
include("dbstring.php");
...
while($row=mysqli_fetch_array($_SQL_2,MYSQLI_ASSOC)){
	...
	echo "</td>";
	echo "<td align='center'>";
	echo "<a href='upload-class-score-entry.php?class_ID=$row[class_entryid]&term_ID=$row[termname]&batch_ID=$row[batchid]&subject_ID=$row[subjectid]'><i class='fa fa-plus' style='color:blue'></i></a>";
	echo "</td>";
	echo "</tr>";
	}
echo "</table>";
...
?>
<form id="formID2" method="post" action="import-class-scores-data.php"  enctype="multipart/form-data">
